...
class DotIndicatorController(context: Context) : IndicatorController, LinearLayout(context) {
    ...
    override var selectedIndicatorColor = ContextCompat.getColor(context, R.color.onboard_default_selected_color)
        set(value) {
            field = value
    ...
    override fun initialize(slideCount: Int) {
        ...
        for (i in 0 until slideCount) {
            ...
            dot.setImageDrawable(
                ContextCompat.getDrawable(this.context, R.drawable.ic_onboard_indicator)
            )
